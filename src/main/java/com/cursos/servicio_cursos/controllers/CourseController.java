...
@RequestMapping("/api/courses") // Anotación para definir la ruta base para todas las solicitudes relacionadas con cursos
...
public class CourseController {
    ...
    public ResponseEntity<CourseEntity> createCourse (@RequestBody RequestCourse requestCourse) { // toma la peticion http y lo convierte en un objeto de tipo RequestCourse, que es un DTO que contiene los datos necesarios para crear un curso
        CourseEntity newCourse = courseService.createCourse(requestCourse);
        return new ResponseEntity<>(newCourse, HttpStatus.CREATED); // Devuelve el curso creado con el estado HTTP 201 (CREATED) aun no entiendo bien lo de los estados. 
    }
